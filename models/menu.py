...
class Menu:
    ...
    def __call__(self, *args, **kwargs):
        ...
        # Выводим первое меню
        while True:
            if start_search:   # Возможно этого блока со start можно избежать
                # query: str = 'django'
            ...
            choice = self.menu_interaction(self.__menu['titles'][0], self.__menu['level_1'])
            match choice:
                case '1':  # Поиск по запросу на HeadHunter
                    temp = search_or_filter.search_processing(class_names=['HeadHunterAPI'], query=query)
                    if not temp:
                        continue
            ...
            while True:
                choice = self.menu_interaction(self.__menu['titles'][1], self.__menu['level_2'])
                match choice:
                    case '1':  # Новый запрос
                ...
                while True:
                    ...
                    #     # filter_keyword: str = 'Москва'
                    #     filter_keyword: str = input("\nВведите слова для фильтра: ").strip()
                    #     start_filter = False

                    choice = self.menu_interaction(self.__menu['titles'][2], self.__menu['level_3'])
                    match choice:
                        case '1':  # Новый фильтр
